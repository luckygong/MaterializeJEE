			...
			<form id="form" class="col s12 right-alert" method="post">
				<div class="row">
					...
					<div class="input-field col s8 m4 s4">
						...
						<input id="parentName" name="parent.name" type="text" value="${(parentName==null?'':parentName)}" disabled> 
						<input id="parentId" name="parent.id" class="hide" type="text" class="validate" value="${(parentId==null?'':parentId)}"> 
						<label for="parentName" class="<c:if test='${parentName!=null}'>active</c:if>">父资源</label>
					</div>
...
$(document).ready(
		function() {
			...
			$("#type").initSelect({"category" : "RESOURCE_TYPE", "emptyOptionTxt" : "-请选择资源类型-", "onlyShowArray":typeShowArray,"defaultSelected":'${type}'});
			$("#searchType").initSelect({"category" : "RESOURCE_TYPE", "emptyOptionTxt" : "-请选择资源类型-"});
			...
			$("#form").validate({
				submitHandler : function(form) {
					...
					$("#form").ajaxSubmit({
						type : "post",
						url : getProjectName() + "/admin/resource/save",
						dataType : "json",
